...
const Post = styled.li<{ currentPost: boolean }>`
  position: relative;
  font-size: 12.8px;
  color: ${props =>
    props.currentPost ? props.theme.colors.text : props.theme.colors.tertiaryText};
  ...
  return (
    <SeriesWrapper>
      ...
      {showViewButton && (
        ...
        >
          {fold ? `View More (+${series.length - filteredPosts.length})` : 'View Less'}
        </ViewMore>
      )}
    </SeriesWrapper>
  );
