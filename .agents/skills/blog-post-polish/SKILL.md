...
description: |
  ...
  Use for src/content/posts or src/content/series markdown when polishing,
  proofreading, fixing Korean phrasing, frontmatter, callouts, markdown hygiene,
  code block formatting, or bilingual term notation. Also use on explicit
  invocation with $blog-post-polish.
---
...
- Translation tasks without a polish intent (use `blog-post-translate` for KO→EN)
...
- **Not a translator** — Korean → English is `blog-post-translate`.
