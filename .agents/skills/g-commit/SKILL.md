...
description: |
  ...
  logical purpose, detects the repo's existing commit style, and stages plus
  commits in dependency order. Supports --dry-run preview mode. Use when asked to
  commit changes, split atomic commits, or preview a commit plan. Also use on
  explicit invocation with $g-commit.
---
