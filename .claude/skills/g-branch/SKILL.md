...
description: |
  ...
  type (feat/fix/chore/docs/refactor/test/style) from Korean or English keywords
  and emits `{type}/{kebab-case}` with a ready-to-run `git checkout -b` command.

  AUTO-TRIGGER when the user asks to:
  - create / suggest / generate a branch name (브랜치명, 브랜치 이름, 브랜치 만들어줘)
  - name a branch for a task they describe in Korean or English
  - infer a branch name from current work (현재 작업, 지금 하는 일 기반으로 브랜치)

  Also triggers on explicit invocation: /g-branch

  SKIP when:
  - The user wants to actually create & switch to a branch without naming help
  - The user is asking about branch strategy / git flow (not naming)
  - The user wants commit or PR work (use `g-commit` or `g-pr`)
---
